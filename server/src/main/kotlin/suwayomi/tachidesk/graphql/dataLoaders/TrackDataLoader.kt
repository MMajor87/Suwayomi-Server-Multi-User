                ...
                transaction {
                    ...
                    val trackRecordsByMangaId =
                        TrackRecordTable
                            .selectAll()
                            .where {
                                (TrackRecordTable.mangaId inList ids) and
...
class DisplayScoreForTrackRecordDataLoader : KotlinDataLoader<Int, String> {
    ...
    override fun getDataLoader(graphQLContext: GraphQLContext): DataLoader<Int, String> {
        ...
        return DataLoaderFactory.newDataLoader<Int, String> { ids ->
            future {
                transaction {
                    addLogger(Slf4jSqlDebugLogger)
                    val trackRecords =
                        TrackRecordTable
                            .selectAll()
                            .where {
                                (TrackRecordTable.id inList ids) and
                                    (TrackRecordTable.userId eq userId)
                            }
                            .toList()
                            .map { it.toTrack() }
                ...
                transaction {
                    ...
                    val trackRecordsBySyncId =
                        TrackRecordTable
                            .selectAll()
                            .where {
                                (TrackRecordTable.trackerId inList ids) and
                                    (TrackRecordTable.userId eq userId)
                            }
                            .map { TrackRecordType(it) }
